...
void handle_client(int sockfd, struct sockaddr_in *client_addr, socklen_t client_len) {
    char buffer[BUF_SIZE];  // buffer pour recevoir messages
    int read_size;

    while (1) {
        // Recevoir un message du client
        read_size = recvfrom(sockfd, buffer, BUF_SIZE, 0, (struct sockaddr *)client_addr, &client_len); 
        if (read_size < 0) {
        ...
        // Analyser la commande et traiter comme dans votre code précédent
        // Exemple de traitement d'une commande "AJOUT"
        ...
        if (sscanf(buffer, "AJOUT %d %d %s %lf", &id_client, &id_compte, password, &somme) == 4) {  
            ...
            if (client && client->compte.id_compte == id_compte && strcmp(client->password, password) == 0) {
                ajouter_solde(client, somme);
                sendto(sockfd, "OK\n", 3, 0, (struct sockaddr *)client_addr, client_len); 
            } else {
                sendto(sockfd, "KO\n", 3, 0, (struct sockaddr *)client_addr, client_len);  
            }
        ...
        // SOLDE
            ...
            if (client && client->compte.id_compte == id_compte && strcmp(client->password, password) == 0) {  
                ...
                sendto(sockfd, response, strlen(response), 0, (struct sockaddr *)client_addr, client_len);  // Envoi UDP
            } else {
                sendto(sockfd, "KO\n", 3, 0, (struct sockaddr *)client_addr, client_len);  // Envoi UDP de KO
            }
        }
            ...
            if (client && client->compte.id_compte == id_compte && strcmp(client->password, password) == 0) {  
                ...
                for (int i = 9; i >= 0; i--) {  // donner en sens inverse pour afficher les plus récentes en premier
                    if (strlen(client->compte.operations[i]) > 0) {
                        ...
                        sendto(sockfd, operation_message, strlen(operation_message), 0, (struct sockaddr *)client_addr, client_len);  // Envoi UDP
                    }
                }

                sendto(sockfd, "FIN\n", 3, 0, (struct sockaddr *)client_addr, client_len);  // Envoi UDP de FIN
            } else {
                sendto(sockfd, "KO\n", 3, 0, (struct sockaddr *)client_addr, client_len);  // Envoi UDP de KO
            }
        } else {
            sendto(sockfd, "COMMANDE INVALIDE\n", 18, 0, (struct sockaddr *)client_addr, client_len);  // Envoi UDP de commande invalide
        }
...
int main() {
    ...
    if (server_sock < 0) {
        perror("Échec de l'initialisation du serveur");
        return 1;
    }
    ...
    while (1) {
        handle_client(server_sock, &client_addr, client_len);  // traiter la demande
    }
